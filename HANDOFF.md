...
- **客户端的地址不能多**：接口上留着别的地址（例如换 `peer_address` 之后没删掉旧的那个）时，内核会拿旧地址当源地址，而 wireguard-go 对源地址不在 allowed-ips 里的包**静默丢弃**——只在 verbose 级别记一行，默认日志里什么都看不到，`wg-stats` 却是「收字节在涨、发出去为零」。2026-09-12 踩过：换成 peer 地址 10.66.66.3 的实例后，接口上还留着 10.66.66.2/32，现象和「隧道没通」一模一样。多实例并存时记得每次只留一个地址。
