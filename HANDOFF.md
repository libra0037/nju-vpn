...
**新怀疑：单会话限制 + 残留会话占位。**
登录响应里 `Is_enable_mult_client` 为 0，即服务端只允许同一账号一个客户端。
本项目反复探测创建过多次隧道会话（23:32 与 23:33 两次成功），
若服务端不因 TCP 断开而立即释放，就会一直返回 ServerReset。
这与「换账号也失败」不矛盾——第二个账号在测试期间可能同样被占用，
或同样积累过会话。

**因此最干净的一次验证是：用一个从未被本项目用过的账号，在手机热点下跑一次。**
这样同时排除「残留会话」与「校园网来源」两个变量。
