...
**「共享源 IP 冲突」假设也已排除（2026-09-10 02:00）**。
用户提供了一台大陆独占 IP 的个人服务器（上海华为云，SS 代理），
Clash 规则让 vpn.nju.edu.cn 固定走它。实测：

- 出口 IP 稳定单一（连续 3 次相同）；
- 经该代理访问服务端握手耗时约 84ms（直连 8ms），确认确实绕道；
- 完整登录 + 短信验证通过后，query-ip 连续三次返回 03（ServerReset）。

因此既不是共享 IP，也不是校园网来源，也不是代理质量问题。

**同批排除的其他项**：

- **ClientHello 形态**：用真实 token 对比「压缩[0]/[1,0] × 有无心跳扩展」
  四种组合，响应完全相同（都是 08）。形态不是决定因素。
- **TLS 1.3 回显问题**：曾怀疑 portal 连接协商 TLS 1.3、
  ServerHello.SessionId 只是回显客户端 session id（那样 token 就是垃圾）。
  实测 ServerHello 与 ClientHello 的 session id **不同**，服务端确实自行生成，
  token 派生是正确的。
- **ServerReset 之后是否还有后续报文**：读一次得到 36 字节后立即 EOF，
  服务端发完控制帧就关闭连接。

**目前唯一未测试的变量：网络来源类型。**
已知成功的组合只有「官方客户端 + 手机热点（移动/家宽 IP）」。
本项目试过的所有路径（校园网、香港集群、日本节点、大陆云服务器）都是
教育网或数据中心 IP，全部失败。

因此下一步应当在**移动网络 / 家宽出口**下跑本客户端。若不便把台式机接热点，
可让笔记本（已连热点）跑一个 SOCKS5 或 HTTP 代理，台式机的 config.yaml
把 proxy 指向它即可，无需迁移仓库。
