...
> 待办清单，不是文档。全部条目已落地或已决定不做（见下），核对完请删除本文件。

## 完成情况（2026-09-12）

| 条目 | 状态 | 落在 |
|---|---|---|
| M1 / M2 / M5 端点按实例派生、显式 -config 失败即退出 | 已做 | 1fb28df |
| M3 报错指路；子进程立刻退出时打印日志尾部 | 已做 | 1fb28df、c07a78f |
| M4 实例身份进日志与 status / ping | 已做 | 07b8947 |
| C1 配置文件权限收紧到 0600 | 已做 | 5c45765 |
| C2 代理 URL 里的凭据脱敏 | 已做 | 38069ca |
| C3 凭据不入日志的回归测试（含 IPC 请求行） | 已做 | b1ce64c |
| C4 / C5 start 一条命令走完、取消 auth 子命令 | 已做 | 09fb226、b1ce64c |
| C6 probe 前先探活拦一次 | 已做 | 38069ca |
| R1 承载端口在登录之前预检 | 已做 | 5f8f991 |
| R2 先登出再关监听 | 已做 | 5f8f991 |
| R3 status -check、start 幂等、error 收敛时给出恢复命令 | 已做 | 5f8f991 |
| R4 auth_pending 超时 | 已做 | b1ce64c |
| R5 子进程退出对 CLI 可见 | 已做 | c07a78f |
| R6 stop 打断正在进行的操作 | 已做 | fd3d9bb |
| P1 / P2 / P3 重试策略与配额、TwfID 不丢、被踢文案 | 已做 | 38069ca |
| L1 / L2 / L3 /tmp 目录校验、拿不到 SID 拒绝启动、写回并发 | 已做 | 1fb28df、38069ca |
| L4 日志名带配置名 / -proxy 透传 / log.level 校验 / listen_port: 0 / 文档与代码对齐 | 已做 | 07b8947、c07a78f、fd3d9bb |
| L4 里的「428 独立退出码」 | 不做 | 见下 |

**决定不做的两条**

- 428 独立退出码：C4 / C5 之后没有任何用户可见命令会收到 428——start 把它当流程的一部分继续走，其余命令不会返回它。等真加回交互式子命令再说。
- TLS pinning：本来就在第 6 节「明确不做」里。

**清单之外、顺手修掉的**

- HTTP Transport 的 DialContext 绕过了 ctx 感知的拨号：portal 请求卡在拨号上时只能等 Transport 超时，stop 与退出都打断不了（fd3d9bb）。
- awaitAuth 的说明文字还在教用户敲已经删掉的 auth 子命令（b1ce64c）。
